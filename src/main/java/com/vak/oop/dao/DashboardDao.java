...
public record DashboardDao(EntityManager em) {
  ...
  public String getBestSellerForMonth(int month, int year) {
    TypedQuery<String> query = em.createQuery("SELECT e.pdname FROM ExportEntity e WHERE MONTH(e.date) = :month AND YEAR(e.date) = :year " + "GROUP BY e.pdname ORDER BY SUM(e.pdquantity) DESC", String.class);
    query.setParameter("month", month);
    query.setParameter("year", year);
  ...
  public String getBestSellerForYear(int year) {
    TypedQuery<String> query = em.createQuery("SELECT e.pdname FROM ExportEntity e WHERE YEAR(e.date) = :year " + "GROUP BY e.pdname ORDER BY SUM(e.pdquantity) DESC", String.class);
    query.setParameter("year", year);
    query.setMaxResults(1);
    return query.getResultStream().findFirst().orElse("-");
  }
  ...
  public List<Object[]> getBestSellerData(int month, int year) {
    TypedQuery<Object[]> query = em.createQuery("SELECT e.pdname, SUM(e.pdquantity) FROM ExportEntity e WHERE MONTH(e.date) = :month AND YEAR(e.date) = :year " + "GROUP BY e.pdname ORDER BY SUM(e.pdquantity) DESC", Object[].class);
    query.setParameter("month", month);
    query.setParameter("year", year);
