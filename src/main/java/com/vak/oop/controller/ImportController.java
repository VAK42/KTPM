...
public class ImportController implements Initializable {
  ...
  @FXML
  private MFXTableColumn<ImportEntity> pdtype;
  @FXML
  ...
  private void setCol() {
    ipid.setRowCellFactory(_ -> new MFXTableRowCell<>(importEntity -> String.valueOf(importEntity.getIpid())));
    pdname.setRowCellFactory(_ -> new MFXTableRowCell<>(ImportEntity::getPdname));
    pdprice.setRowCellFactory(_ -> new MFXTableRowCell<>(importEntity -> String.valueOf(importEntity.getPdprice())));
    pdtype.setRowCellFactory(_ -> new MFXTableRowCell<>(ImportEntity::getPdtype));
    pdquantity.setRowCellFactory(_ -> new MFXTableRowCell<>(importEntity -> String.valueOf(importEntity.getPdquantity())));
    date.setRowCellFactory(_ -> new MFXTableRowCell<>(importEntity -> String.valueOf(importEntity.getDate())));
  }

  private void setWidth() {
    ipid.prefWidthProperty().bind(importTable.widthProperty().multiply(0.1));
    pdname.prefWidthProperty().bind(importTable.widthProperty().multiply(0.25));
    pdprice.prefWidthProperty().bind(importTable.widthProperty().multiply(0.15));
    pdtype.prefWidthProperty().bind(importTable.widthProperty().multiply(0.15));
    pdquantity.prefWidthProperty().bind(importTable.widthProperty().multiply(0.15));
    date.prefWidthProperty().bind(importTable.widthProperty().multiply(0.2));
  }
  ...
  private void deleteSelectedImport() {
    ImportEntity selectedImport = importTable.getSelectionModel().getSelectedValue();
    if (selectedImport != null) {
      ...
      confirmAlert.showAndWait().ifPresent(response -> {
        if (response == ButtonType.OK) {
          importService.deleteImport(selectedImport);
          loadImports();
        }
      });
    } else {
  ...
  private void exportToExcel() {
    ...
    try (org.apache.poi.xssf.usermodel.XSSFWorkbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook()) {
      ...
      String[] headers = {"IMPORT ID", "PRODUCT NAME", "PRICE", "TYPE", "QUANTITY", "DATE"};
      ...
      for (int i = 0; i < imports.size(); i++) {
        ...
        createCell(row, 1, imp.getPdname(), cellStyle);
        createCell(row, 2, imp.getPdprice(), cellStyle);
        createCell(row, 3, imp.getPdtype(), cellStyle);
        createCell(row, 4, imp.getPdquantity(), cellStyle);
        createCell(row, 5, imp.getDate().toString(), cellStyle);
      }
      for (int i = 0; i < headers.length; i++) {
